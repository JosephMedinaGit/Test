...
class Database(common.BaseObject):
    ...
    def profiling_level(self, session=None):
        ...
        .. mongodoc:: profiling
        """
        result = self.command("profiles", -1, session=session)
    ...
    def set_profiling_level(self, level, slow_ms=None, session=None):
        ...
        :Parameters:
          - `level`: Specifies a profiling level, see list of possible values
            below.
          - `slow_ms`: Optionally modify the threshold for the profiles to
        ...
        if slow_ms is not None:
            self.command("profiles", level, slowms=slow_ms, session=session)
        else:
            self.command("profiles", level, session=session)
    ...
    def profiling_info(self, session=None):
        ...
        .. mongodoc:: profiling
        """
        return list(self["system.profiles"].find(session=session))
